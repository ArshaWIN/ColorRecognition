...
import com.mihailenko.ilya.colorrecognizer2016.models.ColorRGB;

import java.util.ArrayList;

/**
 * Created by user1403 on 08.04.16.
 */
public class ColorUtils {

    private static ColorUtils INSTANSE = new ColorUtils();

    public static ColorUtils getInstanse() {
        return INSTANSE;
    }

    private ArrayList<ColorName> defaultColorList;
    private ArrayList<ColorName> spreadColorList;
    ...
    private void initSpreadColorList() {
        ArrayList<ColorName> colorList = new ArrayList<>();
        ...
        spreadColorList = colorList;
    }


    public String getColorNameFromRgb(int r, int g, int b, String type) {
        ArrayList<ColorName> colorList;

        switch (type) {
            case Constants.DEFAULT_TYPE: {
                colorList = defaultColorList;
                break;
            }
            case Constants.EXTRA_TYPE: {
                colorList = spreadColorList;
                break;
            }
            default: {
                colorList = null;
    ...
    public String getColorNameFromHex(int hexColor, String type) {
        int r = (hexColor & 0xFF0000) >> 16;
        int g = (hexColor & 0xFF00) >> 8;
        int b = (hexColor & 0xFF);
        return getColorNameFromRgb(r, g, b, type);
    }

    public String getColorHexFromRGB(int redValue, int greenValue, int blueValue) {
        return String.format("#%02x%02x%02x", redValue, greenValue, blueValue);
    }

    public static ColorRGB getColorRGBFromHex(String colorStr) {
        return new ColorRGB(
